...
const Topics = () => {
    const { fid, did, year, sid } = useParams();
    const [subject, setSubject] = useState([]);
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`http://localhost:8000/api/topics/${fid}/${did}/${year}/${sid}/`); // Replace 'your_api_endpoint' with your actual API endpoint
                    ...
                    <div className="grid gap-4">
                        {
                            subject.map(
                                (sub) => (
                                    <CourseCard key={sub.id} title={sub.topic} linkTo={`/studentdashboard/${did}/${year}/${sid}/${sub.id}`} />
                                )
                            )
                        }
                    </div>
